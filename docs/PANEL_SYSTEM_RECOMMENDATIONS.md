...
| Three attempts before hints | The default is three failed attempts. The free basic hint unlocks immediately; students may then optionally purchase a protected, level-specific detailed hint for 30 XP. Both remain subject to the teacher master switch and configurable 1–10 threshold. | Game page, progress API, challenge settings | Implemented |
...
| Various multimedia elements for sustained game learning | Phaser graphics, sprites and animation, interactive Monaco coding, dialogue, visual feedback, contextual gameplay SFX, quiet chapter-mapped background music, and soft RPG-style dialogue voice blips are supported. Gameplay SFX reinforce existing actions such as scans, pickups, flame and ward activation, shield impacts, healing, spirit release, stair growth, success, and in-world failure through a centralized registry with cooldown and loop cleanup safeguards. Dialogue uses `medium-text-blip-dialogue.mp3` at a controlled every-third-letter cadence. Both systems follow the live, persisted SFX mute/volume controls. Classroom resources support PDFs/images/Office/text/audio/small-video files and external video links. | Game shell, centralized BGM/gameplay/dialogue SFX systems, and classroom resources | Implemented/existing |
...
- A detailed hint costs 30 XP and is optional. The backend checks access, threshold, teacher controls, balance, and prior purchase inside the purchase flow.
