...
const sendTeacherInviteEmail = async ({
  ...
  await getTransport().sendMail({
    ...
    text: `Hi ${firstName || "Teacher"},\n\nAn administrator created a SharpRunner teacher account for you.\n\nUsername: ${username}\nTemporary password: ${temporaryPassword}\n\nSign in here: ${loginUrl}\n\nOn your first login, we will email you a six-digit verification code to activate your account. For security, change your temporary password when password management becomes available. If you were not expecting this invitation, contact your administrator.`,
    html: `<div style="font-family:Arial,sans-serif;color:#23384d;line-height:1.6;max-width:600px;margin:0 auto"><div style="background:#26547c;color:#ffffff;border-radius:10px 10px 0 0;padding:20px 24px"><strong style="font-size:20px">SharpRunner Teacher Invitation</strong></div><div style="border:1px solid #d9e1ea;border-top:0;border-radius:0 0 10px 10px;padding:24px"><p>Hi ${safeName},</p><p>An administrator created a SharpRunner teacher account for you. Use these details to sign in:</p><div style="background:#eef4f8;border-radius:8px;padding:16px 18px;margin:18px 0"><p style="margin:0 0 8px"><strong>Username:</strong> ${safeUsername}</p><p style="margin:0"><strong>Temporary password:</strong> ${safePassword}</p></div><p style="text-align:center;margin:24px 0"><a href="${safeLoginUrl}" style="display:inline-block;background:#26547c;color:#ffffff;text-decoration:none;padding:12px 20px;border-radius:6px;font-weight:700">Log in to SharpRunner</a></p><p>On your first login, we will send a six-digit verification code to this email address. Enter it on SharpRunner to activate your account.</p><p style="color:#687786;font-size:13px">If you were not expecting this invitation, contact your administrator.</p></div></div>`,
  });
};

module.exports = { sendTeacherInviteEmail, sendVerificationEmail };
