...
Classroom, student, lesson, and date controls share one canonical query builder. The same query string is used for dashboard refreshes and CSV requests. Classroom IDs remain ownership-validated; an explicit student ID must be an active student in the teacher's selected active classroom scope, and an unauthorized or malformed ID returns 400 rather than broadening the result. The student selector retains the full authorized roster while the data payload is narrowed to the selected student. Custom date inputs are parsed as exact `YYYY-MM-DD` UTC boundaries (`00:00:00.000Z` through `23:59:59.999Z`) so browser or server local time cannot shift the range.
